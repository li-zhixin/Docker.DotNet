...
public sealed class DockerClient : IDockerClient
{
    ...
    // Key used to signal Microsoft.Net.Http.Client to disable chunked response parsing (for Portainer compatibility)
#if NET5_0_OR_GREATER
    private static readonly HttpRequestOptionsKey<bool> DisableChunkedResponseKey = new("DockerDotNetDisableChunkedResponse");
#else
    private const string DisableChunkedResponseKey = "DockerDotNetDisableChunkedResponse";
#endif
    ...
    #region Portainer WebSocket Support
    ...
    private HttpRequestMessage PrepareRequest(HttpMethod method, string path, IQueryString queryString, IDictionary<string, string> headers, IRequestContent data)
    {
        ...
        // Portainer: Disable chunked response for logs endpoint
        if (ShouldDisableChunkedResponse(path))
        {
#if NET5_0_OR_GREATER
            request.Options.Set(DisableChunkedResponseKey, true);
#else
            request.Properties[DisableChunkedResponseKey] = true;
#endif
        }
